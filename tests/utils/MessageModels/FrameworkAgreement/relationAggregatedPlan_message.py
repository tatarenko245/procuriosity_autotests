...
class RelationApMessage:
    ...
    def build_expected_message_for_relationAp_process(self):
        if "X-OPERATION-ID" in self.__actual_message:
            ...
            if is_operation_id_correct is True:
                ...
                raise ValueError("The message of RelationAp process is not correct: "
                                 "'X-OPERATION-ID' must be uuid.")
        else:
            raise KeyError("The message of RelationAp process is not correct: mismatch key 'X-OPERATION-ID'.")
        ...
        if "X-RESPONSE-ID" in self.__actual_message:
            ...
            if is_process_id_correct is True:
                ...
                raise ValueError("The message of RelationAp process is not correct: 'X-RESPONSE-ID' must be uuid.")
        else:
            raise KeyError("The message of RelationAp process is not correct: mismatch key 'X-RESPONSE-ID'.")

        if "initiator" in self.__actual_message:
            self.__message['initiator'] = "platform"
        else:
            raise KeyError("The message of RelationAp process is not correct: mismatch key 'initiator'.")

        if "ocid" in self.__actual_message['data']:
            self.__message['data']['ocid'] = self.__ap_ocid
        else:
            raise KeyError("The message of RelationAp process is not correct: mismatch key 'data.ocid'.")

        if "url" in self.__actual_message['data']:
            self.__message['data']['url'] = f"{self.tender_url}/{self.__ap_cpid}/{self.__ap_ocid}"
        else:
            raise KeyError("The message of RelationAp process is not correct: mismatch key 'data.url'.")
        ...
        if "operationDate" in self.__actual_message['data']:
            ...
            if is_date_correct is True:
                ...
                raise ValueError("The message of RelationAp process is not correct: 'data.operationDate'.")
        else:
            raise KeyError("The message of RelationAp process is not correct: mismatch key 'data.operationDate'.")
