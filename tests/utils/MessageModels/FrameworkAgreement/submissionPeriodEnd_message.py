""" Prepare expected message for platform, the Submission Period End process of Framework Agreement procedure."""
import copy
import fnmatch
...
class SubmissionPeriodEndMessage:
    ...
    def build_expected_message(self):
        ...
        outcomes_qualifications_array = list()
        for obj in range(self.__expected_quantity_of_outcomes_submission):
            outcomes_qualifications_array.append(copy.deepcopy(self.__message['data']['outcomes']['qualifications'][0]))

            is_qualifications_id_correct = is_it_uuid(
                self.__actual_message["data"]["outcomes"]["qualifications"][obj]["id"]
            )

            if is_qualifications_id_correct is True:

                outcomes_qualifications_array[obj]['id'] = \
                    self.__actual_message["data"]["outcomes"]["qualifications"][obj]["id"]
            else:
                raise ValueError(f"The message is not correct: 'data.outcomes.qualifications[{obj}].id'.")

            is_qualifications_token_correct = is_it_uuid(
                self.__actual_message["data"]["outcomes"]["qualifications"][obj]["X-TOKEN"]
            )

            if is_qualifications_token_correct is True:

                outcomes_qualifications_array[obj]['X-TOKEN'] = \
                    self.__actual_message["data"]["outcomes"]["qualifications"][obj]["X-TOKEN"]
            else:
                raise ValueError(f"The message is not correct: 'data.outcomes.qualifications[{obj}].X-TOKEN'.")

        self.__message['data']['outcomes']['qualifications'] = outcomes_qualifications_array
