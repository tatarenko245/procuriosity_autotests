...
class MessageForPlatform:
    ...
    # def is_message_of_create_ei_process_correct(self, test_mode=False):
    #     budget_url = None
    ...
    #
    #     try:
    #         if "X-OPERATION-ID" in self.message:
    #             check_x_operation_id = is_it_uuid(self.message["X-OPERATION-ID"])
    #     except KeyError:
    #         raise KeyError('KeyError: X-OPERATION-ID')
    #
    #     try:
    #         if "X-RESPONSE-ID" in self.message:
    #             check_x_response_id = is_it_uuid(self.message["X-RESPONSE-ID"])
    #     except KeyError:
    #         raise KeyError('KeyError: X-RESPONSE-ID')
    #     try:
    #         if "initiator" in self.message:
    #             check_initiator = fnmatch.fnmatch(self.message["initiator"], "platform")
    #     except KeyError:
    #         raise KeyError('KeyError: initiator')
    #     try:
    #         if test_mode is False:
    #             check_oc_id = fnmatch.fnmatch(self.message["data"]["ocid"], "ocds-t1s2t3-MD-*")
    #         else:
    #             check_oc_id = fnmatch.fnmatch(self.message["data"]["ocid"], "test-t1s2t3-MD-*")
    ...
    #     try:
    #         if "url" in self.message["data"]:
    #             check_url = fnmatch.fnmatch(self.message["data"]["url"],
    #                                         f"{budget_url}{self.message['data']['ocid']}")
    #     except KeyError:
    #         raise KeyError('KeyError: url')
    #     try:
    #         if "operationDate" in self.message["data"]:
    #             check_operation_date = fnmatch.fnmatch(self.message["data"]["operationDate"], "202*-*-*T*:*:*Z")
    #     except KeyError:
    #         raise KeyError('KeyError: operationDate')
    #     try:
    #         if test_mode is False:
    #             check_ei_id = fnmatch.fnmatch(self.message["data"]["outcomes"]["ei"][0]["id"], "ocds-t1s2t3-MD-*")
    #         else:
    #             check_ei_id = fnmatch.fnmatch(self.message["data"]["outcomes"]["ei"][0]["id"], "test-t1s2t3-MD-*")
    #
    ...
    #     try:
    #         if "X-TOKEN" in self.message["data"]["outcomes"]["ei"][0]:
    #             check_ei_token = is_it_uuid(self.message["data"]["outcomes"]["ei"][0]["X-TOKEN"])
    #     except KeyError:
    ...
    #
    # @staticmethod
    # def create_bid_message_is_successful(environment, kafka_message, pn_ocid, tender_id):
    #     tender_url = None
    #     if environment == "dev":
    #         tender_url = "http://dev.public.eprocurement.systems/tenders"
    #     if environment == "sandbox":
