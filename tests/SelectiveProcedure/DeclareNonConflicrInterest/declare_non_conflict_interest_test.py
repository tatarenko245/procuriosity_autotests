...
from tests.utils.PayloadModel.RestrictedProcedure.DeclareNonConflictInterest.declare_non_conflict_interest_prepared_payload import \
...
class TestDeclareNonConflictInterest:
    ...
    def test_check_pn_ms_releases_one(self, get_hosts, country, language, pmd, environment, connection_to_database):
        ...
        for i in submission_period_end_feed_point_message['data']['outcomes']['qualifications']:
            for q in actual_tp_release_after_submission_period_end_expired['releases'][0]['qualifications']:
                if q['status'] == "pending" and q['statusDetails'] == "awaiting":

                    with allure.step(f'# {step_number}. Authorization platform one: create qualification'):
                        """
                        Tender platform authorization for create tender phase process.
                        As result get Tender platform's access token and process operation-id.
                        """
                        create_qualification_access_token = authorization.get_access_token_for_platform_one()
                        create_qualification_operation_id = authorization.get_x_operation_id(
                            create_qualification_access_token)
                        step_number += 1

                    with allure.step(f'# {step_number}. Send request to create qualification.'):
                        """
                        Send api request on BPE host for create qualification.
                        Save synchronous result of sending the request and asynchronous result of sending the request.
                        """
                        time.sleep(1)
                        qualification_payload_class = copy.deepcopy(QualificationPreparePayload())

                        create_qualification_payload = \
                            qualification_payload_class.create_qualification_obligatory_data_model(
                                status_details="active")

                        print("Номер кваліфікейшена")
                        print(i['id'])
                        Requests().create_qualification(
                            ...
                            qualification_token=i['X-TOKEN'],
                            payload=create_qualification_payload,
                            test_mode=True)

                        create_qualification_feed_point_message = KafkaMessage(
                            create_qualification_operation_id).get_message_from_kafka()
                        step_number += 1
                        print("create_qualification_feed_point_message")
                        print(create_qualification_feed_point_message)
