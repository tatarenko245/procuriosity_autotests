...
class TestCreateCn:
    @allure.title("Check Ev and MS releases data after Submission creating without optional fields. \n"
                  "------------------------------------------------\n"
    ...
    def test_check_pn_ms_releases_one(self, get_hosts, country, language, pmd, environment, connection_to_database):
        ...
        with allure.step(f'# {step_number}. See result'):
            ...
            with allure.step(f'# {step_number}.2. Check message in feed point'):
                ...
                """
                create_submission_feed_point_message = MessageForPlatform(
                    create_submission_operation_id).get_message_from_kafka_topic()
